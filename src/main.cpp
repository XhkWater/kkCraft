...
static void enableDarkTitleBar(SDL_Window *window) {
  ...
  // Try current attribute first, then fallback for older Windows builds.
...
public:
  ...
  void initWindow() {
    ...
    window =
        SDL_CreateWindow("kkCraft", SDL_WINDOWPOS_CENTERED,
  ...
  void run() {
    ...
    while (running) {
      ...
      const float deltaSeconds =
          static_cast<float>(currentTicks - previousTicks) /
...
private:
  ...
  void createImageViews() {
    ...
    for (size_t i = 0; i < swapchainImages.size(); ++i) {
      ...
      vkCheck(vkCreateImageView(device, &createInfo, nullptr,
  ...
  void createDepthResources() {
    ...
    vkCheck(vkCreateImage(device, &imageInfo, nullptr, &depthImage),
  ...
  void createGraphicsPipeline() {
    const std::string vertexShaderPath =
        std::string("shaders") + "/triangle.vert.spv";
    const std::string fragmentShaderPath =
        std::string("shaders") + "/triangle.frag.spv";
    ...
    // Disable culling to avoid winding-order mismatch across coordinate spaces.
    ...
    vkCheck(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr,
                                   &pipelineLayout),
  ...
  void createFramebuffers() {
    ...
    for (size_t i = 0; i < swapchainImageViews.size(); ++i) {
      ...
      vkCheck(vkCreateFramebuffer(device, &framebufferInfo, nullptr,
                                  &framebuffers[i]),
  ...
  void createVertexBuffer() {
    ...
    void *data = nullptr;
    vkCheck(vkMapMemory(device, vertexBufferMemory, 0, bufferInfo.size, 0,
                        &data),
  ...
  void createSyncObjects() {
    ...
    vkCheck(vkCreateSemaphore(device, &semaphoreInfo, nullptr,
                              &imageAvailableSemaphore),
            "Failed to create imageAvailable semaphore");
    vkCheck(vkCreateSemaphore(device, &semaphoreInfo, nullptr,
                              &renderFinishedSemaphore),
            "Failed to create renderFinished semaphore");
    vkCheck(vkCreateFence(device, &fenceInfo, nullptr, &inFlightFence),
  ...
  void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) const {
    ...
    clearValues[0].color.float32[3] = 1.0f; // sky blue
    ...
    if (windowAspect > targetAspect) {
      ...
      viewportX =
          (static_cast<float>(swapchainExtent.width) - viewportWidth) * 0.5f;
    } else if (windowAspect < targetAspect) {
      viewportHeight = viewportWidth / targetAspect;
      viewportY =
          (static_cast<float>(swapchainExtent.height) - viewportHeight) * 0.5f;
    }
    ...
    VkRect2D scissor{};
    scissor.offset = {static_cast<int32_t>(viewportX),
                      static_cast<int32_t>(viewportY)};
    scissor.extent = {static_cast<uint32_t>(viewportWidth),
    ...
    const glm::mat4 view = glm::lookAt(cameraPosition, cameraTarget, worldUp);
    glm::mat4 proj = glm::perspective(glm::radians(45.0f), targetAspect, 0.1f,
                                      10.0f);
    proj[1][1] *= -1.0f;
  ...
  void drawFrame() {
    ...
    // Only reset the fence when we are going to submit work this frame.
